function PopularCategories() {
    ...
    return (
      <div className="mt-16 px-6 sm:px-12 md:px-24">
        ...
        {/* Grid layout: 2 rows, 6 columns on medium screens and above, 2 columns on small screens */}
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-4 sm:gap-6">
          {categories.map((category) => (
            <div
              key={category.id}
              className="flex flex-col items-center border border-gray-200 rounded-lg p-4 transition-transform duration-300 hover:scale-105 hover:shadow-lg"
            >
              <img
                src={category.image}
